...
Sift 采用三层管道架构，数据源、处理逻辑、投递渠道完全解耦，用户反馈自动驱动偏好演进：
...
Sift 不需要手动选择"关注领域"——系统从你的行为中自动学习偏好。
...
每次运行周报，系统会自动将文章存入本地 SQLite 数据库（`workspaces/default/knowledge.db`），逐步构建你的技术知识库：
...
- 单文件存储（`workspaces/default/knowledge.db`），零运维，手机上直接跑
